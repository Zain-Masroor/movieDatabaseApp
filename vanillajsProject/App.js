    ...
    if (movies.length === 0) {
        ...
        movies.forEach(movie => {
            ...
            movieCard.innerHTML = `
                ...
                ${movie.streamingUrl ? `<a class="watch-btn" href="${movie.streamingUrl}" target="_blank">Watch Now</a>` : ''}
            `;
            ...
            // Add functionality to click on movie card and go to the streaming URL
  ...
  document.getElementById('addMovieBtn').addEventListener('click', function() {
    ...
    if (movieName && genre && year && rating) {
        const newMovie = {
            ...
            streamingUrl: streamingUrl ? streamingUrl : null // Store URL if provided
        };
        ...
        document.getElementById('streamingUrl').value = ''; // Clear the URL input
    } else {
